...
def test_compute_loss_returns_three_tensors() -> None:
    ...
    total, pl, vl, _, _ = compute_loss(log_policy, value, policy_target, value_target)
    assert total.shape == ()
    assert pl.shape == ()
    assert vl.shape == ()
...
def test_perfect_policy_gives_zero_policy_loss() -> None:
    ...
    value_target = torch.zeros(batch)
    _, pl, _, _, _ = compute_loss(log_policy, value, policy_target, value_target)
    assert pl.item() >= 0.0
...
def test_perfect_value_gives_zero_value_loss() -> None:
    ...
    value_target = torch.tensor([-1.0, 0.0, 1.0, -1.0])
    _, _, vl, _, _ = compute_loss(log_policy, value_target, log_policy.exp(), value_target)
